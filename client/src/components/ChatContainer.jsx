...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
          ...
          {onlineUsers.includes(selectedUser) && <span className='w-2 h-2 rounded-full bg-green-500'></span>}
        </p>
        ...
        {messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.sender !== authUser._id ? 'flex-row-reverse' : ''}`}>
            ...
            <div className='text-center text-xs'>
              <img src={msg.sender === authUser._id ? authUser ?.profilePic 
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-lg font-medium text-white'>By K.Bharathraj</p>
    </div>
  )
}
